...
```python
# Current: 14D state (eef pos + eef quaternion + arm joints)
@ProcessorStepRegistry.register("libero_processor")
class LiberoProcessorStep(ObservationProcessorStep):
    def _process_observation(self, obs):
        eef_pos = robot_state["eef"]["pos"]          # 3D
        eef_quat = robot_state["eef"]["quat"]        # 4D
        joints = robot_state["joints"]["pos"]        # 7D
        state = torch.cat([eef_pos, eef_quat, joints], dim=-1)  # 14D
        return state
...
class LiberoProcessorStep(ObservationProcessorStep):
    ...
    **State Processing:**
    ...
    - Extracts end-effector quaternion (4D)
    - Extracts arm joint positions (7D)
    - Concatenates into 14D state vector
    ...
    **Image Processing:**
    ...
    def _process_observation(self, observation):
        ...
        # Process robot_state: Flatten to 14D vector
        if "observation.robot_state" in processed_obs:
            ...
            state = torch.cat((eef_pos, eef_quat, joints_pos), dim=-1)
            state = state.float()
...
2. **State Flattening**: The raw LIBERO environment exposes nested dictionaries with all available state information (position, quaternion, velocity, matrix representation, etc.). The processor:
   - Selects the relevant components (eef pos, eef quat, arm joints)
   - Keeps quaternion orientation and arm joint positions
   - Flattens to a single 14D vector that policies expect
