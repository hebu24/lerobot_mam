...
# Match the mixed-mask protocol used by long52/long70. The repeated 3D_points
# entry intentionally gives dense XYZ guidance two of the four train slots.
export DATASET_REPO_ID=local/libero10_mam_v3_refmix_train
export DATASET_ROOT="${DATASET_ROOT:-data/libero10_mam/libero10_mam_v3_refmix_train}"
export MAM_EVAL_DATASET_REPO_ID=local/libero10_mam_v3_refmix_eval
...
# Keep avgmse and all long64 settings unchanged.
...
export JOB_NAME="${JOB_NAME:-mam_libero10_refmix_train4_eval5_long64_avgmse_scratch_8gpu_${RUN_ID}}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/train/${JOB_NAME}}"

echo "MAM long64 mixed-mask training from scratch"
...
exec bash scripts/run_mam_libero10_conda.sh \
  --policy.allow_independent_eval_source=false \
