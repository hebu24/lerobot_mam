...
# Restart the continuation from the same 90k random-mask base checkpoint, but
# train on the 500-episode refmix dataset and evaluate on the independent
# rollout source. Eight GPUs at batch/GPU=12 preserve effective batch size 96.
export DATASET_REPO_ID="${DATASET_REPO_ID:-local/libero10_500_refmix_train}"
export DATASET_ROOT="${DATASET_ROOT:-data/libero10_mam/libero10_500_refmix_train}"
export MAM_EVAL_DATASET_REPO_ID="${MAM_EVAL_DATASET_REPO_ID:-local/libero10_100first50_refmix_eval}"
...
if [[ "${STEPS}" -ne 200000 ]]; then
  echo "This experiment must end at global step 200000, got ${STEPS}." >&2
  exit 2
fi

RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
export JOB_NAME="${JOB_NAME:-mam_libero10_500train_100first50eval_refmix_long64_avgmse_resume${step_id}to200000_8gpu_${RUN_ID}}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/train/${JOB_NAME}}"

echo "MAM long64 8-GPU continuation retrain"
